 ...
 *
 * The canonical request string layout is a cross-language contract, pinned by
 * the golden vectors in `test-fixtures/service-auth-vectors.json`. Those
 * vectors are produced by an independent Python reference
 * (`test-fixtures/generators/service_auth_reference.py`) so this file cannot
 * validate itself. Any change to the layout or the canonicalization rules
 * requires a new format tag (`sig2`), not an edit here.
 */
 ...
 *
 * Returns `X-OpenOutposts-Service`, `X-OpenOutposts-Service-Signature`, plus
 * `X-OpenOutposts-Actor` when an actor is asserted and `x-trace-id` when a

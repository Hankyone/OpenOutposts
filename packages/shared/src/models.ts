...
/**
 * Extract provider and model from a model ID, or null when the ID names no
 * provider.
 *
 * Normalizes bare Claude and GPT names first — those two prefixes are a
 * documented migration for IDs already stored without a provider — then splits
 * on "/". Anything left without a "/" names no provider, and this returns null
 * rather than guessing one.
 *
 * The guess this replaced was `anthropic`. Provider derivation feeds credential
 * resolution, so an unknown ID quietly asked for the wrong provider's key and
 * failed somewhere further downstream with no mention of the model. Callers
 * must now say what they want to happen when the provider is unknown, and every
 ...
 * extractProviderAndModel("some-unknown-model") // null
 */
export function extractProviderAndModel(
  ...
  return model.length > 0 ? { provider, model } : null;
}

/**
 * Whether an ID is a usable model reference: `provider/model-id`, or a bare
 * Claude/GPT name that normalizes into one.
 *
 * This is deliberately weaker than `isValidModel`. The hardcoded catalog below
 * answers "is this one of the models this package knows about", which a homestead
 * catalog legitimately exceeds; this answers "can this ID be resolved to a
 * provider at all", which every path needs regardless of who owns the list.
 */
export function isModelReference(modelId: string): boolean {
  return extractProviderAndModel(modelId) !== null;
}
 ...
 *
 * This substitutes silently by design and must only be used where the caller
 * expressed no preference — a stored fallback, a UI placeholder. It is never
 * the right way to resolve a model a user actually chose: the substitution is
 * invisible, so the user is told one model and served another. Paths that
 * resolve a chosen model refuse instead and name the model.
 */

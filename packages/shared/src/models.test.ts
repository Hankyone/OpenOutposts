...
describe("model utilities", () => {
  ...
  it("refuses to invent a provider for an ID that names none", () => {
    // Provider derivation feeds credential resolution. Guessing "anthropic"
    // here is how an unknown model quietly asked for the wrong provider's key.
    expect(extractProviderAndModel("unknown-model")).toBeNull();
    expect(extractProviderAndModel("")).toBeNull();
    expect(extractProviderAndModel("/model-with-no-provider")).toBeNull();
    expect(extractProviderAndModel("provider-with-no-model/")).toBeNull();
  });

  it("recognizes model references independently of the hardcoded catalog", () => {
    // A homestead catalog legitimately offers models this package has never named,
    // so reference-shape and catalog-membership are separate questions.
    expect(isModelReference("anthropic/claude-sonnet-4-6")).toBe(true);
    expect(isModelReference("some-provider/some-model-the-list-never-named")).toBe(true);
    expect(isModelReference("claude-opus-4-8")).toBe(true);
    expect(isValidModel("some-provider/some-model-the-list-never-named")).toBe(false);
    expect(isModelReference("unknown-model")).toBe(false);
  });

...
import { tolerantSandboxEventsSchema } from "./sandbox-events";
...
describe("boundary schemas", () => {
  ...
  describe("sandboxEventSchema", () => {
    ...
    /**
     * The control plane shortens an oversized event before storing it and
     * marks the stored copy. These schemas strip what they do not name, so
     * without the field the marker would survive storage and then be dropped
     * on the way back to the client — leaving a fragment that reads as the
     * whole output.
     */
    describe("the storage truncation marker", () => {
      const cases = [
        {
          type: "tool_call",
          tool: "bash",
      ...
      for (const partial of cases) {
        it(`survives a ${partial.type} round trip`, () => {
          const event = {
            ...partial,
            messageId: "message-1",
            sandboxId: "sandbox-1",
          ...
          const [parsed] = tolerantSandboxEventsSchema.parse([event]);
          expect(parsed).toBeDefined();
          expect((parsed as { truncated?: boolean }).truncated).toBe(true);
        });
      }

      it("is absent on an event that was stored whole", () => {
        const [parsed] = tolerantSandboxEventsSchema.parse([
          {
            type: "tool_result",
            callId: "call-1",
            result: "all of it",
        ...
        expect((parsed as { truncated?: boolean }).truncated).toBeUndefined();
      });
    });
  });

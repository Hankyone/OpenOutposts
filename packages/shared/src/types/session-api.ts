...
const createSessionRequestBaseSchema = z.object({
  ...
  /**
   * Pin the session to a specific outpost. Only meaningful on deployments
   * using the outpost execution backend; omitted sessions use the
   * deployment's default target. Orthogonal to the repository target.
   */
  outpostId: z
    .string()
    .trim()
...
export const spawnContextSchema = z.object({
  ...
  /** Immutable execution target inherited by every descendant. */

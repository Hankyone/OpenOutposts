...
export interface SessionState {
  ...
  /**
   * The outpost this session executes on, or null when the session runs on a
   * provisioned sandbox. Resolved at read time: a session pinned to a machine
   * carries its own id, and an unpinned session on an outpost deployment
   * carries the deployment's target. The name and heartbeat are not repeated
   * here — the fleet listing owns them, so a machine that is renamed or goes
   * offline needs no session-state rewrite.
   */
...
export const sessionStateSchema = z.object({
  ...
  // The machine this session executes on. Optional so pre-feature producers
  // stay valid — absent means "unknown", which the UI renders as the sandbox

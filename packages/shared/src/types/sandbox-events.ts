...
/**
 * Set on the stored copy of an event whose payload was too large to keep
 * whole. Live events never carry it — the control plane broadcasts what
 * arrived and shortens only what it keeps — so this is what tells a replayed
 * event apart from the one the user watched arrive.
 *
 * It has to be declared here rather than tolerated as an unknown key: these
 * schemas strip what they do not name, so without it the marker would survive
 * storage and then be thrown away on the way back to the client, leaving a
 * shortened output that reads as a complete one.
 */
const truncatedForStorageSchema = z.boolean().optional();

...
the cross-language contract between ``service_auth_reference.py`` (which
generates it, via this script) and ``service-auth.ts`` (which asserts it
byte-for-byte). Run from the repository root:

    python3 packages/shared/test-fixtures/generators/generate_service_auth_vectors.py
...
from service_auth_reference import (
    SIG1_PREFIX,
    _canonical_pathname,
...
FIXTURE_PATH = Path(__file__).resolve().parents[1] / "service-auth-vectors.json"
...
DESCRIPTION = (
    ...
    "packages/shared/test-fixtures/generators/service_auth_reference.py. "
    "Changing canonicalization requires a sig2, not an edit to these vectors. "
    "Regenerate with "
    "packages/shared/test-fixtures/generators/generate_service_auth_vectors.py."
)
...
MALFORMED_HEADERS: list[dict[str, str]] = [
    ...
    {
        "name": "non-ASCII digit timestamp",
        "signatureHeader": f"sig1.١٢٣.{_NONCE}.{_SIG}",
    },
    {"name": "negative timestamp", "signatureHeader": f"sig1.-1.{_NONCE}.{_SIG}"},
    {"name": "zero timestamp", "signatureHeader": f"sig1.0.{_NONCE}.{_SIG}"},
    {
        "name": "17-digit timestamp",
        "signatureHeader": f"sig1.{'9' * 17}.{_NONCE}.{_SIG}",
    },
    {
        "name": "uppercase nonce",
        "signatureHeader": f"sig1.{_TS}.{_NONCE.upper()}.{_SIG}",
    },
    {
        "name": "uppercase signature",
        "signatureHeader": f"sig1.{_TS}.{_NONCE}.{_SIG.upper()}",
    },
    {
        "name": "truncated signature",
        "signatureHeader": f"sig1.{_TS}.{_NONCE}.{_SIG[:32]}",
    },
]
...
def main() -> None:
    fixture = {
        ...
        "malformedHeaders": [
            {**case, "reason": "format"} for case in MALFORMED_HEADERS
        ],
    }
    FIXTURE_PATH.write_text(json.dumps(fixture, indent=2, ensure_ascii=False) + "\n")
    print(
        f"wrote {len(VECTOR_INPUTS)} vectors, {len(MALFORMED_HEADERS)} malformed headers"
    )
    print(FIXTURE_PATH)

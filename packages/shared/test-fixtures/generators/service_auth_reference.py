"""Independent Python reference for the ``sig1`` signature format.

This file exists so the golden vectors in
``packages/shared/test-fixtures/service-auth-vectors.json`` keep being produced
by an implementation that is NOT the one asserting them. ``service-auth.ts``
verifies the fixture byte-for-byte; if the fixture were generated from the same
code it verifies, the check would be a tautology.

It was the sandbox runtime's ``service_auth.py`` until the in-sandbox runtime
was removed. It is a reference, not a runtime dependency: nothing imports it
outside the generator next to it.

The canonical request string layout is a cross-language contract; any change to
the layout or the canonicalization rules requires a new format tag (``sig2``),
not an edit here.
...
# Mirrors TOKEN_VALIDITY_SECONDS in packages/shared/src/auth.ts.
TOKEN_VALIDITY_SECONDS = 5 * 60

SERVICE_HEADER = "X-OpenOutposts-Service"
SERVICE_SIGNATURE_HEADER = "X-OpenOutposts-Service-Signature"
ACTOR_HEADER = "X-OpenOutposts-Actor"
SIG1_PREFIX = "sig1"
...
def verify_service_signature(
    *,
    ...
    return ServiceSignatureResult(
        ok=True, reason=None, timestamp_ms=timestamp_ms, nonce=nonce
    )

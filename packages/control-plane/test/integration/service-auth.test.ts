...
import { seedSessionOutpost } from "./helpers";
...
describe("sig1 service-credential authentication", () => {
  ...
  it("refuses a replayed signature, and the refusal outlives the isolate's memory", async () => {
    ...
    // The signature is valid, so the first delivery is accepted.
    const first = await SELF.fetch(url, { headers });
    expect(first.status).toBe(200);

    // Byte-identical replay, still inside the validity window.
    const replay = await SELF.fetch(url, { headers });
    expect(replay.status).toBe(401);

    // The refusal must come from a durable record, not from a Map an isolate
    // could lose or a second isolate never see.
    const row = await env.DB.prepare("SELECT COUNT(*) AS count FROM service_auth_nonces").first<{
      count: number;
    }>();
  ...
  it("persists bot session ownership from the signed actor", async () => {
    const actor = await new UserStore(env.DB).resolveOrCreateUser({
      provider: "slack",
      providerUserId: "U0001",
      providerLogin: "slack-U0001",
    });
    await seedSessionOutpost(actor.id);

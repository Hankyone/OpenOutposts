...
describe("POST /internal/prompt", () => {
  ...
  it("refuses a per-prompt model that names no provider, naming it", async () => {
    // The old behaviour dropped the override with a server-side log line and
    // ran the prompt on the session's previous model. A well-formed model the
    // harness cannot serve is refused by the harness, in the turn, by name.
    ...
    expect(body.error).toContain("not-a-model-reference");

    const messages = await queryDO<{ id: string }>(stub, "SELECT id FROM messages");

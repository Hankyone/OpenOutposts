...
describe("DO internal sub-session routes", () => {
  ...
  describe("POST /internal/cancel", () => {
    ...
    it("stops sandbox when cancelling an active session", async () => {
      ...
      // Simulate a live sandbox on a running session. Session creation kicks a
      // spawn that cannot complete here (no homestead is connected to the test
      // control plane), so the row is written directly.
      await queryDO(stub, "UPDATE sandbox SET status = 'running'");
    ...
    it("stops sandbox when spawn failure lands before cancel", async () => {
      const { stub } = await initSession({
        repoOwner: "acme",
        repoName: "web-app",
        userId: "user-1",

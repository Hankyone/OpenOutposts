...
describe("AutomationStore (D1 integration)", () => {
  ...
  describe("list", () => {
    it("lists all automations", async () => {
      ...
      const result = await store.list({ ownerUserId: null });
    ...
    it("filters by repo owner and name via repository rows", async () => {
      ...
      const result = await store.list({ ownerUserId: null, repoOwner: "acme", repoName: "api" });
    ...
    it("matches multi-repository automations on any selected repository", async () => {
      ...
      const byApi = await store.list({ ownerUserId: null, repoOwner: "acme", repoName: "api" });
      expect(byApi.automations.map((a) => a.id)).toEqual(["auto-multi"]);
      const byWeb = await store.list({ ownerUserId: null, repoOwner: "acme", repoName: "web" });
      expect(byWeb.automations.map((a) => a.id)).toEqual(["auto-multi"]);
      const byOther = await store.list({ ownerUserId: null, repoOwner: "acme", repoName: "other" });
    ...
    it("returns only one owner's automations when scoped, and excludes unowned rows", async () => {
      const store = new AutomationStore(env.DB);
      await store.create(makeAutomation({ id: "auto-alice", user_id: "user-alice" }));
      await store.create(makeAutomation({ id: "auto-bob", user_id: "user-bob" }));
      await store.create(makeAutomation({ id: "auto-orphan", user_id: null }));

      const result = await store.list({ ownerUserId: "user-alice" });
      expect(result.automations.map((a) => a.id)).toEqual(["auto-alice"]);
      expect(result.total).toBe(1);
    });
  });

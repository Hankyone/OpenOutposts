 ...
 * the per-key import from a member repo, which re-seals each value from the
 * repo's scope into the environment's.
 *
 * The import used to copy ciphertext verbatim — both scopes share one
 * encryption key, so it worked and kept plaintext out of the control plane.
 * Values are now bound to the scope that stores them, so a verbatim copy would
 * land a row nothing can read. The test below asserts the new behaviour, and
 * asserts the source ciphertext genuinely does not decrypt in the destination
 * scope, which is the property the binding exists for.
 */
...
import {
  ...
  repoSecretScope,
  scopedSecretContext,
} from "../../src/auth/encryption-contexts";
...
describe("EnvironmentSecretsStore", () => {
  ...
  it("imports selected repo secrets, re-sealed into the environment's scope", async () => {
    ...
    // Not the source bytes: the value was re-sealed for this environment.
    expect(copied?.encrypted_value).not.toBe(srcCipher.get("DB_URL"));
    // It reads as the original plaintext under the environment's own scope.
    expect(
      await decryptToken(
        copied!.encrypted_value,
        key(),
        scopedSecretContext(environmentSecretScope(ENV_ID), "DB_URL")
      )
    ).toBe("postgres://secret");
    expect(await envStore.getDecryptedSecrets(ENV_ID)).toEqual({ DB_URL: "postgres://secret" });

    // And the reason a verbatim copy is no longer possible: the repo's own
    // ciphertext does not authenticate in the environment's scope.
    await expect(
      decryptToken(
        srcCipher.get("DB_URL")!,
        key(),
        scopedSecretContext(environmentSecretScope(ENV_ID), "DB_URL")
      )
    ).rejects.toThrow();
    // It still reads in the scope it was written for.
    expect(
      await decryptToken(
        srcCipher.get("DB_URL")!,
        key(),
        scopedSecretContext(repoSecretScope(42), "DB_URL")
      )
    ).toBe("postgres://secret");

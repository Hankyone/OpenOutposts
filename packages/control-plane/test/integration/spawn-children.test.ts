...
import { initNamedSession, queryDO, seedSandboxAuth, seedSessionOutpost } from "./helpers";
...
describe("POST /sessions/:parentId/children — spawn child", () => {
  ...
  async function setupParent(opts?: {
    ...
    const canonicalUserId =
      opts?.canonicalUserId === undefined ? (opts?.userId ?? null) : opts.canonicalUserId;
    if (canonicalUserId) {
      await seedSessionOutpost(canonicalUserId);
    }
  ...
  it("rejects a disabled model override for grandchildren", async () => {
    const { parentName, sandboxToken } = await setupParent({ userId: "user-1" });
  ...
  it("refuses a child when the parent has no canonical owner", async () => {
    const { parentName, sandboxToken, store } = await setupParent({
      repoId: 12345,
      userId: "user-1",

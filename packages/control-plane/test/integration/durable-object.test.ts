...
import { applyMigrations, MIGRATIONS } from "../../src/session/schema";
...
describe("SessionDO Durable Object", () => {
  ...
  it("replays migration 39 after its singleton seed was recorded", async () => {
    const id = env.SESSION.newUniqueId();
    const stub = env.SESSION.get(id);
    ...
    const initResponse = await stub.fetch("http://internal/internal/init", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sessionName: "test-session-migration-39-replay",
        repoOwner: "acme",
        repoName: "api",
        repoId: 39,
        userId: "user-migration-39",
      }),
    });
    expect(initResponse.status).toBe(200);
    ...
    await runInDurableObject(stub, (instance: SessionDO) => {
      ...
      const initialRows = sql.exec("SELECT singleton FROM events_meta").toArray();
      expect(initialRows).toHaveLength(1);

      sql.exec("UPDATE events_meta SET pruned_count = 7 WHERE singleton = 1");
      sql.exec("DELETE FROM _schema_migrations WHERE id = 39");

      expect(() => applyMigrations(sql)).not.toThrow();

      const rows = sql.exec("SELECT singleton, pruned_count FROM events_meta").toArray() as Array<{
        singleton: number;
        pruned_count: number;
      }>;
      expect(rows).toEqual([{ singleton: 1, pruned_count: 7 }]);

      const migrationRows = sql
        .exec("SELECT id FROM _schema_migrations WHERE id = 39")
        .toArray() as Array<{ id: number }>;
      expect(migrationRows).toEqual([{ id: 39 }]);
    });
  });

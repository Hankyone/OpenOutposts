...
import { createSqlDatabaseStub } from "./db/sql-database-stub";
...
vi.mock("./auth/outpost-ownership", () => ({
  requireSessionOutpost: vi.fn(async (_ctx, requestedOutpostId, defaultOutpostId) => ({
    id: requestedOutpostId ?? defaultOutpostId ?? "outpost-test",
  })),
}));
...
describe("handleSpawnChild prompt enqueue handling", () => {
  ...
  it("inherits a parent model the bundled list has never named, rather than defaulting it", async () => {
    // The parent may legitimately run a model only a homestead catalog names.
    // Re-validating an inherited model against the bundled list is what
    // silently downgraded such a child to the deployment default.
    const unlistedContext = { ...spawnContext, model: "anthropic/claude-tiny-experimental" };
    const store = makeStore("canonical-user-123", unlistedContext);
    vi.mocked(SessionIndexStore).mockImplementation(function () {
      return store as never;
    });

    const parentStub: DurableObjectStub = {
      fetch: vi.fn(async () => Response.json(unlistedContext)),
    } as never;
  ...
  it("returns 400 when child specifies an invalid model", async () => {
    ...
    // Model resolution reads the homestead model catalog; an empty catalog is the

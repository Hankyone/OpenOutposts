...
async function handleListRepos(
  ...
  try {
    ...
    if (e instanceof SourceControlProviderError && e.errorType === "permanent" && !e.httpStatus) {
      // Having no source control configured is a deployment state, not a
      // server fault — an outpost-only deployment is a supported way to run.
      // Report it as an empty list the caller can explain, matching how the
      // background refresh above already treats this case.
      logger.warn("SCM provider not configured, returning an empty repo list", {
        trace_id: ctx.trace_id,
      });
      return json({ repos: [], cached: false, cachedAt: null, scmConfigured: false });
    }
...
async function handleListBranches(
  ...
  try {
    ...
    if (e instanceof SourceControlProviderError && e.errorType === "permanent" && !e.httpStatus) {
      // Same deployment state as the repo list: no provider, so no branches.
      return json({ branches: [], scmConfigured: false });
    }

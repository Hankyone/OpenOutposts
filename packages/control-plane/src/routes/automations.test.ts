...
/**
 * A D1 stub that answers every read with no rows. Model resolution consults
 * the homestead model catalog on every create/update; an empty catalog is the
 * "unchecked" case, where the bundled model list answers instead.
 */
function createStubDb(): SqlDatabase {
  const statement = {
    bind: () => statement,
    first: async () => null as never,
    run: async () => ({ results: [] as never[], meta: { changes: 0 } }),
    all: async () => ({ results: [] as never[], meta: { changes: 0 } }),
  };
  return { prepare: () => statement, batch: mockBatch } as unknown as SqlDatabase;
}
...
describe("automation route handlers", () => {
  ...
  describe("GET /automations (list)", () => {
    ...
    it("leaves a service principal's listing deployment-wide", async () => {
      mockStore.list.mockResolvedValue({ automations: [], total: 0 });

      await callRoute("GET", "/automations", { principal: SLACK_BOT_PRINCIPAL });

      expect(mockStore.list).toHaveBeenCalledWith(expect.objectContaining({ ownerUserId: null }));
    });

    it("refuses rather than listing everything when a user has no canonical id", async () => {
      const res = await callRoute("GET", "/automations", {
        principal: {
          kind: "user",
          user: {
  ...
  describe("POST /automations (create)", () => {
    ...
    it("refuses a model this deployment does not offer, instead of storing the default", async () => {
      // Storing the deployment default instead would silently retarget every
      // future firing of the automation.
      const res = await callRoute("POST", "/automations", {
        body: { ...validBody, model: "anthropic/not-a-real-model" },
      });
    ...
    it("stores the product default when no model was chosen", async () => {
      const res = await callRoute("POST", "/automations", {
        body: { ...validBody, model: undefined },
      });

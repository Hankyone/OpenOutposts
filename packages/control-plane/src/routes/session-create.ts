...
async function handleCreateSession(
  ...
  //
  // On the outpost path the harness's own catalog decides what exists, so a
  // model the session owner cannot reach is refused here with a reason rather
  // than silently rewritten to the deployment default and discovered as a dead
  // session at the first prompt. The managed-sandbox path has no catalog and
  // keeps the hardcoded list — but refuses just as loudly against it.
  ...
  if (!resolvedModel.ok) {
    logger.info("session.model_unreachable", {
      event: "session.model_unreachable",
      user_id: resolvedUserId,
      requested_model: body.model ?? undefined,
      request_id: ctx.request_id,
      trace_id: ctx.trace_id,
    });
    return error(resolvedModel.error, 400);
  }
  if (resolvedModel.substituted) {
    // Nobody chose a model, so applying one is not a swap of anyone's choice —
    // but it is still recorded, so "which model ran and who picked it" always
    // has an answer.
    logger.info("session.model_defaulted", {
      event: "session.model_defaulted",
      user_id: resolvedUserId,
      selected_model: resolvedModel.model,
      catalog_governed: resolvedModel.catalogGoverned,
      request_id: ctx.request_id,
      trace_id: ctx.trace_id,
    });
  }

...
import { automationOwnerFilterFor } from "../auth/automation-ownership";
...
async function handleListAutomations(
  ...
  // `/automations` names no automation, so the router's ownership gate does not
  // cover it (the same split as `GET /sessions`). The listing scopes itself
  // instead: a user credential sees only its own rows, a service principal the
  // whole deployment.
  const ownerUserId = automationOwnerFilterFor(ctx);
  if (ownerUserId instanceof Response) return ownerUserId;

  const store = new AutomationStore(ctx.db);
  const result = await store.list({ ownerUserId, repoOwner, repoName });
...
async function handleCreateAutomation(
  ...
  if (triggerType === "webhook") {
    ...
    triggerAuthData = await encryptSentrySecret(sentrySecret, env.REPO_SECRETS_ENCRYPTION_KEY, id);
  }
  ...
  // Model resolution sits after the owner is known: on the outpost path
  // reachability is per user, and the automation's owner is the person whose
  // provider credentials the fired session will use. A model that cannot be
  // honoured is refused by name — storing the deployment default instead would
  // silently retarget every future firing of this automation.
  const resolvedModel = await resolveRequestedModel({
    env,
    db: ctx.db,
  ...
  const reasoningEffort = resolveReasoningEffortFor(resolvedModel, body.reasoningEffort);
...
async function handleUpdateAutomation(
  ...
  // An edit that names a model is checked against whichever list this
  // deployment answers to; an edit that does not keeps the stored model
  // verbatim. Re-validating the stored model against the hardcoded list would
  // quietly rewrite an automation that already runs a model only the homestead
  // catalog names, on an edit that never mentioned the model.
  const resolvedModel = await resolveRequestedModel({
    env,
    db: ctx.db,
    userId: existing.user_id,
    requested: body.model ?? null,
    inherited: existing.model,
  });
  if (!resolvedModel.ok) return error(resolvedModel.error, 400);
  const nextModel = resolvedModel.model;
  const requestedReasoningEffort = body.reasoningEffort;
  const resolvedReasoningEffort =
    requestedReasoningEffort !== undefined
      ? resolveReasoningEffortFor(resolvedModel, requestedReasoningEffort)
      : body.model !== undefined && existing.reasoning_effort !== null
        ? resolveReasoningEffortFor(resolvedModel, existing.reasoning_effort)
 ...
 * the automations store.
 *
 * Service principals only. The answer is a union across every user's enabled
 * slack automations and cannot be scoped to a caller without making it useless
 * to the bot that needs the whole set. It sits outside `/automations`, so the
 * router's ownership gate does not reach it; being merely authenticated is not
 * enough, and a signed-in user reading it would learn which Slack channels
 * other people watch.
 */
async function handleGetWatchedSlackChannels(
  ...
  if (ctx.principal?.kind === "user") {
    logger.warn("Auth failed: watched slack channels", {
      event: "auth.automation_ownership_denied",
      reason: "not_service_principal",
      request_id: ctx.request_id,
      trace_id: ctx.trace_id,
    });
    return error("Watched channels are available to service principals only", 403);
  }

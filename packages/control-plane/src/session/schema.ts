...
/**
 * Running totals for the events table, so retention never has to count the
 * whole transcript to decide whether it is over budget. A single row, kept in
 * step inside the same transaction as every event write.
 *
 * `pruned_count` and `pruned_through` are the record of what was let go: how
 * many rows, and the newest `created_at` among them. Nothing reads them today,
 * and they are the deliberate seam for a later deep-history fetch-through —
 * a replay that reaches past `pruned_through` knows it has hit the boundary
 * rather than the beginning of the session.
 */
const EVENTS_META_TABLE_SQL = `CREATE TABLE IF NOT EXISTS events_meta (
...
CREATE TABLE IF NOT EXISTS session (
  ...
  outpost_id TEXT,                                  -- Pinned outpost execution target; NULL uses the deployment default
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
  CHECK (
...
CREATE TABLE IF NOT EXISTS sandbox (
  ...
  auth_token TEXT,                                  -- RETIRED: pre-hash plaintext token. Never read; emptied on wake (SessionDO.retireLegacySandboxAuthToken). Dropped once every DO has woken at least once.
  auth_token_hash TEXT,                             -- SHA-256 hash of sandbox auth token (the only accepted credential)
  credential_fetch_token_hash TEXT,                 -- SHA-256 hash of the model-credential fetch token; that route and no other
...
-- Running size of the events table, so retention never counts the whole
-- transcript to decide whether it is over budget.
${EVENTS_META_TABLE_SQL};
...
export const MIGRATIONS: readonly SchemaMigration[] = [
  ...
  {
    id: 37,
    description: "Add outpost_id to session (pinned execution target)",
    run: `ALTER TABLE session ADD COLUMN outpost_id TEXT`,
  },
  {
    id: 38,
    description: "Add credential_fetch_token_hash to sandbox",
    run: `ALTER TABLE sandbox ADD COLUMN credential_fetch_token_hash TEXT`,
  },
  {
    id: 39,
    description: "Add events_meta running totals for transcript retention",
    run: (sql) => {
      sql.exec(EVENTS_META_TABLE_SQL);
      // Seed from what this session already holds, so a DO that has been
      // running since before retention existed starts with a true size rather
      // than zero — which would let it grow one whole budget further before
      // pruning ever fired. `length(CAST(data AS BLOB))` counts bytes; bare
      // `length()` on TEXT counts characters and would undercount every
      // multibyte payload. `OR IGNORE` handles a replay after the singleton
      // was written but before migration tracking was recorded.
      sql.exec(
        `INSERT OR IGNORE INTO events_meta (singleton, total_count, total_bytes, pruned_count, pruned_through, updated_at)
         SELECT 1, COUNT(*), COALESCE(SUM(length(CAST(data AS BLOB))), 0), 0, NULL, ?
         FROM events`,
        Date.now()
      );
    },

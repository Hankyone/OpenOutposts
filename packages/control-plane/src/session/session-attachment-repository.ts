...
export class SessionAttachmentRepository {
  ...
  /**
   * Every stored object this session's attachments own, referenced or not.
   * Read by the purge, which has to release them all before the rows that
   * name them are wiped.
   */
  listObjectKeys(): string[] {
    const result = this.sql.exec(`SELECT object_key FROM attachments`);
    return (result.toArray() as Array<{ object_key: string }>).map((row) => row.object_key);
  }

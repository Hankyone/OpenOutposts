...
describe("applyMigrations", () => {
  ...
  /**
   * Retention needs to know how big the transcript already is. A DO that has
   * been running since before it existed must start from its real size — a
   * zero would let it grow one whole budget further before pruning ever fired.
   */
  describe("events_meta", () => {
    it("creates the running totals for fresh DOs and migrated DOs alike", () => {
      expect(SCHEMA_SQL).toContain("CREATE TABLE IF NOT EXISTS events_meta");
      expect(SCHEMA_SQL).toContain("pruned_through INTEGER");

      const migration = MIGRATIONS.find((item) => item.id === 39);
      expect(migration).toBeDefined();

      applyMigrations(mock.sql);
      const create = mock.calls.find((c) =>
        c.query.includes("CREATE TABLE IF NOT EXISTS events_meta")
      );
      expect(create).toBeDefined();
    });

    it("seeds the totals from the events the session already holds", () => {
      applyMigrations(mock.sql);

      const seed = mock.calls.find((c) => c.query.includes("INSERT OR IGNORE INTO events_meta"));
      expect(seed).toBeDefined();
      expect(seed?.query).toContain("SELECT 1, COUNT(*)");
      // Bytes, not characters: bare length() on TEXT counts characters and
      // would undercount every multibyte payload in the table.
      expect(seed?.query).toContain("SUM(length(CAST(data AS BLOB)))");
      expect(seed?.query).toContain("FROM events");
    });

    it("does not overwrite totals a previous run already seeded", () => {
      applyMigrations(mock.sql);

      const seed = mock.calls.find((c) => c.query.includes("INSERT OR IGNORE INTO events_meta"));
      expect(seed?.query).toContain("INSERT OR IGNORE INTO events_meta");
    });

    it("is skipped entirely once it has been recorded", () => {
      mock.setData(
        "SELECT id FROM _schema_migrations",
        MIGRATIONS.map((m) => ({ id: m.id }))
      );

      applyMigrations(mock.sql);

      expect(mock.calls.find((c) => c.query.includes("events_meta"))).toBeUndefined();
    });
  });
});

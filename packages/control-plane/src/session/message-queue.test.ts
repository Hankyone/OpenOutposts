...
describe("SessionMessageQueue", () => {
  ...
  describe("model selection", () => {
    it("refuses an override that names no provider", async () => {
      const h = buildQueue();
      ...
      expect(h.wsManager.send).toHaveBeenCalledWith(
        ws,
        expect.objectContaining({ type: "error", code: "INVALID_MODEL" })
      );
      expect(h.repository.createMessageWithAttachments).not.toHaveBeenCalled();
    });

    it("accepts a homestead-catalog model the hardcoded list has never named", async () => {
      // The catalog deliberately contains models the bundled list never heard
      // of. Judging those against the bundled list is the defect.
      const h = buildQueue();
      ...
      expect(h.repository.createMessageWithAttachments).toHaveBeenCalledWith(
        expect.objectContaining({
          model: "anthropic/claude-tiny-experimental",
          // The bundled list cannot judge this model's efforts, so it does not
          // get to discard one the user selected.
          reasoningEffort: "high",
        }),
        []
      );
    ...
    it("stores a bare legacy model id in canonical form", async () => {
      const h = buildQueue();
    ...
    it("dispatches the session's model unchanged when the bundled list has never named it", async () => {
      const h = buildQueue();
      const sandboxWs = { readyState: 1 } as WebSocket;
      h.repository.getSession.mockReturnValue(
        createSession({ model: "anthropic/claude-tiny-experimental" })
      );
      h.repository.getNextPendingMessage.mockReturnValue(createMessage({ id: "msg-99" }));
      h.wsManager.getSandboxSocket.mockReturnValue(sandboxWs);
      ...
      expect(h.wsManager.send).toHaveBeenCalledWith(
        sandboxWs,
        expect.objectContaining({ type: "prompt", model: "anthropic/claude-tiny-experimental" })
      );
    });

    it("dispatches the per-prompt override ahead of the session's model", async () => {
      const h = buildQueue();
      const sandboxWs = { readyState: 1 } as WebSocket;
      h.repository.getNextPendingMessage.mockReturnValue(
        createMessage({ id: "msg-100", model: "openai/gpt-experimental" })
      );
      h.wsManager.getSandboxSocket.mockReturnValue(sandboxWs);
      ...
      expect(h.wsManager.send).toHaveBeenCalledWith(
        sandboxWs,
        expect.objectContaining({ type: "prompt", model: "openai/gpt-experimental" })
      );
    });

    it("applies the product default only when nothing recorded a model, and says so", async () => {
      const h = buildQueue();
      const sandboxWs = { readyState: 1 } as WebSocket;
      h.repository.getSession.mockReturnValue(createSession({ model: "" }));
      h.repository.getNextPendingMessage.mockReturnValue(createMessage({ id: "msg-101" }));
      h.wsManager.getSandboxSocket.mockReturnValue(sandboxWs);
      ...
      expect(h.wsManager.send).toHaveBeenCalledWith(
        sandboxWs,
        expect.objectContaining({ type: "prompt", model: DEFAULT_MODEL })
      );
      expect(h.log.warn).toHaveBeenCalledWith(
        "prompt.dispatch.no_model_recorded",
        expect.objectContaining({ message_id: "msg-101", default_model: DEFAULT_MODEL })
      );
    });
  });

...
export class ParticipantService {
  ...
  /**
   * The SCM identity a participant's stored tokens are sealed to.
   *
   * A row holding SCM ciphertext with no SCM user id is a credential nobody
   * can be named for. There is no safe reading of it — an unbound decrypt is
   * exactly the relocation the binding exists to refuse — so this throws and
   * the caller's existing handler falls back to the app token and logs.
   */
  private scmIdentityOf(participant: ParticipantRow): string {
    if (!participant.scm_user_id) {
      throw new Error("Participant carries SCM tokens but no scm_user_id to bind them to");
    }
    return participant.scm_user_id;
  }
  ...
  private async refreshTokenCentralized(
    ...
    try {
      ...
      if (store.isTokenFresh(d1Tokens.expiresAt)) {
        ...
        await this.updateLocalTokensFromD1(participant.id, scmUserId, d1Tokens);
      ...
      if (casResult.ok) {
        ...
        const newAccessTokenEncrypted = await encryptToken(
          ...
          scmOAuthContext(scmUserId, "access_token")
        );
        const newRefreshTokenEncrypted = await encryptToken(
          newRefreshToken,
          this.env.TOKEN_ENCRYPTION_KEY,
          scmOAuthContext(scmUserId, "refresh_token")
        );
      ...
      if (winnerTokens) {
        await this.updateLocalTokensFromD1(participant.id, scmUserId, winnerTokens);
        return this.repository.getParticipantById(participant.id);
      }
  ...
  private async updateLocalTokensFromD1(
    ...
    const [accessEnc, refreshEnc] = await Promise.all([
      encryptToken(
        d1Tokens.accessToken,
        this.env.TOKEN_ENCRYPTION_KEY,
        scmOAuthContext(scmUserId, "access_token")
      ),
      encryptToken(
        d1Tokens.refreshToken,
        this.env.TOKEN_ENCRYPTION_KEY,
        scmOAuthContext(scmUserId, "refresh_token")
      ),
    ]);
    ...
    try {
      const [accessToken, refreshToken] = await Promise.all([
        decryptToken(
          participant.scm_access_token_encrypted,
          this.env.TOKEN_ENCRYPTION_KEY,
          scmOAuthContext(participant.scm_user_id, "access_token")
        ),
        decryptToken(
          participant.scm_refresh_token_encrypted,
          this.env.TOKEN_ENCRYPTION_KEY,
          scmOAuthContext(participant.scm_user_id, "refresh_token")
        ),
      ]);
  ...
  private async refreshTokenLocal(participant: ParticipantRow): Promise<ParticipantRow | null> {
    ...
    try {
      const scmUserId = this.scmIdentityOf(participant);
  ...
  > {
    ...
    try {
      const accessToken = await decryptToken(
        ...
        this.env.TOKEN_ENCRYPTION_KEY,
        scmOAuthContext(this.scmIdentityOf(resolvedParticipant), "access_token")
      );

...
import { requireSessionOutpost } from "../auth/outpost-ownership";
...
export interface SessionInitInput {
  ...
  /**
   * Pinned outpost execution target. Only meaningful on deployments using
   * the outpost sandbox backend; null falls back to the deployment default.
   */
...
export async function initializeSession(
  ...
  // Resolve and pin the target before creating any session state. Every create
  // path, including child and scheduled sessions, passes through here, so no
  // caller can bypass the ownership boundary by omitting an outpost ID.
  const selectedOutpost = await requireSessionOutpost(
    ctx,
    input.outpostId,
    env.OUTPOST_TARGET_ID,

...
describe("createSessionLifecycleHandler", () => {
  ...
  it("refuses to seal a plain SCM token when no SCM identity is given to bind it to", async () => {
    const { handler, repository, encryptToken, validateReasoningEffort, generateId, log } =
      createHandler();
    validateReasoningEffort.mockReturnValue(null);
    generateId.mockReturnValueOnce("sandbox-1").mockReturnValueOnce("participant-1");
    ...
    const response = await handler.init(
      new Request("http://internal/internal/init", {
        ...
        body: JSON.stringify({
          ...
          repoId: 123,
          userId: "user-1",
          // No scmUserId: nothing to bind the ciphertext to.
          scmToken: "plain-scm-token",
        }),
      })
    );
    ...
    expect(repository.createParticipant).toHaveBeenCalledWith(
      expect.objectContaining({ scmAccessTokenEncrypted: null })
    );
    expect(log.error).toHaveBeenCalledWith(
      "Refusing to store SCM token with no SCM user id to bind it to"
    );
  });

  it("seals a plain SCM token to the SCM identity it belongs to", async () => {
    const { handler, repository, encryptToken, validateReasoningEffort, generateId } =
      createHandler();
    validateReasoningEffort.mockReturnValue(null);
    generateId.mockReturnValueOnce("sandbox-1").mockReturnValueOnce("participant-1");
    encryptToken.mockResolvedValue("sealed-token");
    ...
    expect(encryptToken).toHaveBeenCalledWith("plain-scm-token", expect.any(String), {
      table: "scm_oauth",
      scopeKind: "scm_user",
      scopeId: "9001",
    ...
    expect(repository.createParticipant).toHaveBeenCalledWith(
      expect.objectContaining({ scmAccessTokenEncrypted: "sealed-token" })
    );
  });

  it("stores the model it was handed rather than re-judging it against the bundled list", async () => {
    // Replaces an assertion that this handler rewrites an unrecognised model to
    // the deployment default. The creating route has already checked the model
    // against whichever list this deployment answers to, and a homestead catalog
    // legitimately names models the bundled list does not — so rewriting here
    // silently swapped a model the user had already been shown as selected.
    ...
    expect(repository.upsertSession).toHaveBeenCalledWith(
      expect.objectContaining({
        model: "anthropic/claude-tiny-experimental",
        // The bundled list has no opinion on this model's efforts, so it does
        // not get to discard the one that was chosen.
        reasoningEffort: "high",
      })
    );
  ...
  it("normalizes a bare legacy model id", async () => {
    const { handler, repository, validateReasoningEffort, generateId } = createHandler();
    validateReasoningEffort.mockReturnValue(null);
    generateId.mockReturnValueOnce("sandbox-1").mockReturnValueOnce("participant-1");
  ...
  it("refuses a model id that names no provider instead of defaulting it", async () => {
    ...
    await expect(response.json<{ error: string }>()).resolves.toEqual({
      error: expect.stringContaining("not-a-model-reference"),
    });
    expect(repository.upsertSession).not.toHaveBeenCalled();
  });

  it("applies the product default only when no model was given, and says so", async () => {
    ...
    expect(log.info).toHaveBeenCalledWith(
      "Session created with no model; applying the product default",
  ...
  it("makes cancel authoritative after a spawn failure", async () => {
    const {
      handler,
      getSession,
      getSandbox,
    ...
    getSession.mockReturnValue(createSession({ status: "active" }));
    getSandbox.mockReturnValue(createSandbox({ status: "failed" }));
    stopExecution.mockResolvedValue(undefined);
    transition.mockResolvedValue(true);

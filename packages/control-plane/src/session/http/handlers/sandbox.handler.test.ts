...
function createHandler() {
  ...
  const isValidCredentialFetchToken = vi.fn();
...
describe("createSandboxHandler", () => {
  ...
  it("refuses credential issuance before a session generation becomes active", async () => {
    const { handler, getSandbox, isValidCredentialFetchToken, log } = createHandler();
    getSandbox.mockReturnValue({ status: "pending" } as SandboxRow);
    vi.mocked(isValidCredentialFetchToken).mockResolvedValue(true);

    const response = await handler.verifySandboxToken(
      new Request("http://internal/internal/verify-sandbox-token", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ token: "abc", purpose: "credential_fetch" }),
      })
    );
    ...
    expect(await response.json()).toEqual({
      valid: false,
      error: "Session generation not active",
    });
    expect(isValidCredentialFetchToken).not.toHaveBeenCalled();
    expect(log.warn).toHaveBeenCalledWith(
      "Credential fetch token verification failed: generation is not active",
      { status: "pending" }
    );
  });

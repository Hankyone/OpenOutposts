 ...
 *
 * Seconds since the epoch reach this value in the year 5138; milliseconds
 * passed it in 1973. Every real timestamp on either side of the line is
 * therefore classified correctly, and no plausible date is ambiguous.
 */
const MILLISECOND_TIMESTAMP_FLOOR = 1e11;

/**
 * Put an event's timestamp in the unit the rest of the system uses.
 *
 * Everything the control plane records is in seconds and the web client
 * multiplies by a thousand to render, so a producer sending milliseconds dates
 * its events roughly fifty five thousand years out and sorts them inconsistently
 * against events the control plane made itself. Producers are expected to send
 * seconds; correcting it here means one that does not cannot corrupt the
 * transcript, and a stored event is always in one unit.
 */
function withNormalizedTimestamp<T extends SandboxEventWithAck>(event: T): T {
  if (typeof event.timestamp !== "number" || event.timestamp < MILLISECOND_TIMESTAMP_FLOOR) {
    return event;
  }
  return { ...event, timestamp: event.timestamp / 1000 };
}
...
export class SessionSandboxEventProcessor {
  ...
  constructor(
    ...
    /**
     * How large one stored event may be. Applied only on the way to storage:
     * every broadcast below carries the event exactly as it arrived, because a
     * user watching a command run should see all of what it printed.
     */
  ...
  async processSandboxEvent(incoming: SandboxEventWithAck): Promise<void> {
    const event = withNormalizedTimestamp(incoming);

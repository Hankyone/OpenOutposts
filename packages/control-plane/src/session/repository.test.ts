...
/**
 * The event writes carry running-total accounting alongside the insert, so a
 * test about WHICH row was written has to pick the insert out rather than
 * assume it was the only statement.
 */
function eventInserts(mock: ReturnType<typeof createMockSql>) {
  return mock.calls.filter((call) => call.query.includes("INSERT INTO events ("));
}
...
describe("SessionRepository", () => {
  ...
  describe("updateSandboxStatus", () => {
    ...
    it.each(["stopped", "stale"] as const)(
      "revokes both credentials when status becomes %s",
      (status) => {
        repo.updateSandboxStatus(status);

        expect(mock.calls[0].query).toContain("auth_token_hash = NULL");
        expect(mock.calls[0].query).toContain("credential_fetch_token_hash = NULL");
        expect(mock.calls[0].params).toEqual([status]);
      }
    );
  });

  describe("rotateSandboxCredentials", () => {
    it("rotates both hashes only for the matching active generation", () => {
      mock.setDefaultRowsWritten(1);

      expect(
        repo.rotateSandboxCredentials({
          sandboxId: "generation-1",
          authTokenHash: "rotated-bridge-hash",
          credentialFetchTokenHash: "rotated-fetch-hash",
        })
      ).toBe(true);

      expect(mock.calls[0].query).toContain("modal_sandbox_id = ?");
      expect(mock.calls[0].query).toContain("status IN");
      expect(mock.calls[0].query).toContain("auth_token_hash IS NOT NULL");
      expect(mock.calls[0].query).toContain("credential_fetch_token_hash IS NOT NULL");
      expect(mock.calls[0].params).toEqual([
        "rotated-bridge-hash",
        "rotated-fetch-hash",
        "generation-1",
  ...
  describe("revokeDormantSandboxCredentials", () => {
    it("clears credentials only from dormant generations", () => {
      repo.revokeDormantSandboxCredentials();

      expect(mock.calls[0].query).toContain("status IN ('stopped', 'stale')");
      expect(mock.calls[0].query).toContain("auth_token_hash = NULL");
      expect(mock.calls[0].query).toContain("credential_fetch_token_hash = NULL");
    });
  });
  ...
  describe("updateSandboxForSpawn", () => {
    it("sets all spawn fields atomically", () => {
      repo.updateSandboxForSpawn({
        ...
        credentialFetchTokenHash: "fetch-hash-123",
      ...
      expect(mock.calls[0].query).toContain("credential_fetch_token_hash");
      expect(mock.calls[0].query).toContain("modal_sandbox_id");
  ...
  describe("createEvent", () => {
    it("stores event with all fields", () => {
      ...
      const inserts = eventInserts(mock);
      expect(inserts.length).toBe(1);
      expect(inserts[0].params).toEqual(["evt-1", "tool_call", '{"tool":"read"}', "msg-1", 1000]);
    });
  });
  ...
  describe("upsertTokenEvent", () => {
    it("upserts token event by deterministic message key", () => {
      ...
      const inserts = eventInserts(mock);
      expect(inserts.length).toBe(1);
      expect(inserts[0].query).toContain("VALUES (?, ?, ?, ?, ?)");
      expect(inserts[0].query).toContain("ON CONFLICT(id) DO UPDATE SET");
    ...
    it("reuses the same deterministic ID across updates", () => {
      ...
      expect(inserts[0].params[0]).toBe("token:msg-1");
      expect(inserts[1].params[0]).toBe("token:msg-1");
      expect(inserts[1].params[1]).toBe("token");
      expect(inserts[1].params[2]).toBe(JSON.stringify(secondEvent));
      expect(inserts[1].params[4]).toBe(2000);
    });
  });
  ...
  describe("upsertExecutionCompleteEvent", () => {
    ...
    it("reuses the same deterministic completion ID across updates", () => {
      ...
      const inserts = eventInserts(mock);
      expect(inserts.length).toBe(2);
      expect(inserts[0].params[0]).toBe("execution_complete:msg-1");
      expect(inserts[1].params[0]).toBe("execution_complete:msg-1");
      expect(inserts[1].params[1]).toBe("execution_complete");
      expect(inserts[1].params[2]).toBe(JSON.stringify(secondEvent));
      expect(inserts[1].params[4]).toBe(3000);
    });
  });

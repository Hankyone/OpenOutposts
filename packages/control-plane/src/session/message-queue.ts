...
export class SessionMessageQueue {
  constructor(
    ...
    private readonly retention: EventRetentionConfig
    /**
     * Whether a connected homestead's harness catalog, rather than the hardcoded
     * product list, decides what this deployment can run. It changes which list
     * a per-prompt model override is judged against — never whether an override
  ...
  async handlePromptMessage(
    ...
    try {
      ...
      if (error instanceof PromptModelError) {
        // The refusal goes back down the socket that sent the prompt. Dropping
        // the override and answering on the previous model is the failure this
        // is here to prevent.
  ...
  async processMessageQueue(): Promise<void> {
    ...
    const session = this.repository.getSession();
    // No coercion here. Both candidates were validated where they were chosen —
    // the override at enqueue, the session's model at creation — and running
    // them through the hardcoded list again is what rewrote a legitimate
    // homestead-catalog model to the deployment default at the moment of dispatch,
    // after the UI had already shown the user their choice.
    let resolvedModel = message.model || session?.model || "";
    if (!resolvedModel) {
      // A session that never recorded a model at all. The default is the right
      // answer — nobody chose anything — but it is said out loud rather than
      // arrived at by a fallback chain.
      resolvedModel = DEFAULT_MODEL;
      this.log.warn("prompt.dispatch.no_model_recorded", {
        event: "prompt.dispatch.no_model_recorded",
        message_id: message.id,
  ...
  writeUserMessageEvent(
    ...
    this.repository.createEvent({
      id: generateId(),
      type: "user_message",
      // A user's own message has no field this policy will shorten, so this is
      // a passthrough today. It goes through the same gate as everything else
      // so that stays true by construction rather than by memory.
      data: JSON.stringify(
        boundSandboxEventForStorage(userMessageEvent, this.retention.eventPayloadMaxBytes)
      ),
      messageId,
      createdAt: now,
    });
  ...
  private async enqueuePromptCore(data: EnqueuePromptCoreData): Promise<EnqueuedPrompt> {
    ...
    // Throws PromptModelError, which both callers turn into a visible refusal.
    // Nothing is written and no prompt is queued: a model override that cannot
    // be honoured ends the request rather than downgrading it.
    const messageModel = data.model ? validatePromptModel(data.model) : null;

    const effectiveModelForEffort =
      messageModel || this.repository.getSession()?.model || DEFAULT_MODEL;
    // The hardcoded catalog may only judge efforts for models it names. For a
    // model a homestead offers and this package has never heard of it has no
    // opinion, and inventing one would drop an effort the user selected.
    const messageReasoningEffort = isValidModel(effectiveModelForEffort)
      ? validateReasoningEffort(effectiveModelForEffort, data.reasoningEffort, this.log)

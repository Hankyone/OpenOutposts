  ...
  describe("userAgent threading", () => {
    ...
    it("falls back to the default User-Agent when none is configured", async () => {
      ...
      expect(mockGetInstallationRepository).toHaveBeenCalledWith(
        ...
        "acme",
        "web",
        expect.objectContaining({ userAgent: "OpenOutposts" })
      );
    });
  });
...
const basePullResponse = {
  ...
  head: { ref: "openoutposts/session-1", sha: "abc123", repo: { id: 9001, full_name: "acme/web" } },
  base: {
    ref: "main",
...
describe("getPullRequest", () => {
  ...
  it("reads with app auth and maps the response to a snapshot", async () => {
    ...
    expect(snapshot).toEqual({
      ...
      lifecycleState: "open",
      isDraft: true,
      headBranch: "openoutposts/session-1",
      baseBranch: "main",
      headSha: "abc123",
      repoOwner: "acme",
      repoName: "web",
  ...
  it("captures headSha and repositoryExternalId from the create response", async () => {
    ...
    const result = await provider.createPullRequest(
      { authType: "oauth", token: "user-token" },
      {
        ...
        title: "Add feature",
        body: "Description",
        sourceBranch: "openoutposts/session-1",
        targetBranch: "main",
      }
    );
  ...
  it("leaves capture fields undefined when the response omits them", async () => {
    mockFetchWithTimeout.mockResolvedValueOnce(
      makeJsonResponse({
        ...
        draft: false,
        merged: false,
        head: { ref: "openoutposts/session-1" },
        base: { ref: "main" },
      })
    );

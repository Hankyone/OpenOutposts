...
export class AutomationStore {
  ...
  /**
   * List automations, always scoped to one owner or explicitly to all.
   *
   * `ownerUserId` is required rather than optional: this listing used to run a
   * bare `deleted_at IS NULL` over every row, so an omitted filter means
   * "everyone's automations" — the exact disclosure the owner scope exists to
   * prevent. Making it required forces every call site to say which it wants.
   * Null is the deployment-wide listing a service principal gets.
   */
  async list(options: {

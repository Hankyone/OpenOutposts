 ...
 * table-specific SQL and public API; all three encrypt with
 * REPO_SECRETS_ENCRYPTION_KEY, as before.
 *
 * Values are sealed to their scope and key (see auth/encryption-contexts).
 * Rows written before that binding existed are read unbound and rewritten
 * bound by the next write to their scope.
 */

import { encryptToken, decryptToken } from "../auth/crypto";
import { scopedSecretContext, type SecretScope } from "../auth/encryption-contexts";
import {
 ...
 *
 * `scope` names the store and the row family the values belong to; each value
 * is sealed to that scope plus its own key, so a ciphertext cannot be moved to
 * another repository, another environment, or another key and still decrypt.
 */
export async function encryptSecretEntries(
  ...
  for (const [key, value] of Object.entries(normalized)) {
    ...
    entries.push({
      key,
      encryptedValue: await encryptToken(value, encryptionKey, scopedSecretContext(scope, key)),
    });
  }
    ...
    rows.map(async (row) => {
      try {
        const decryptedValue = await decryptToken(
          row.encrypted_value,
          encryptionKey,
          scopedSecretContext(scope, row.key)
        );

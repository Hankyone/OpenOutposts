...
describe("AutomationStore", () => {
  ...
  describe("list", () => {
    ...
    it("constrains the query to one owner when a scope is given", async () => {
      const { db, statements } = createFakeD1({ allResults: [] });
      const store = new AutomationStore(db);
      await store.list({ ownerUserId: "user-1" });

      expect(statements[0].sql).toContain("user_id = ?");
      expect(statements[0].params).toContain("user-1");
    });

    it("applies no owner predicate for the deployment-wide listing", async () => {
      const { db, statements } = createFakeD1({ allResults: [] });
      const store = new AutomationStore(db);
      await store.list({ ownerUserId: null });

      expect(statements[0].sql).not.toContain("user_id");
    });

    it("combines the owner scope with a repository filter", async () => {
      const { db, statements } = createFakeD1({ allResults: [] });
      const store = new AutomationStore(db);
      await store.list({ ownerUserId: "user-1", repoOwner: "acme", repoName: "api" });

      expect(statements[0].sql).toContain("user_id = ?");
      // Owner first, then the repository EXISTS binds — positional order must
      // match the SQL or the filters silently swap.
      expect(statements[0].params).toEqual(["user-1", "acme", "api"]);
    });
  });

...
export interface AnalyticsFilters {
  ...
  /**
   * Restrict every aggregate to sessions owned by this user, or null for the
   * deployment-wide view.
   *
   * Required, not optional: an omitted owner scope reads as "everyone", which
   * is a cross-user disclosure no caller should be able to select by
   * forgetting. See auth/analytics-scope.ts for who gets which value.
   */
  ownerUserId: string | null;
}

/**
 * The owner predicate and its bind parameters for a `sessions` query.
 *
 * A null scope contributes no predicate at all (deployment-wide). A scoped
 * query matches `user_id` exactly, so sessions with no recorded owner fall
 * outside every user's aggregates rather than being attributed to whoever
 * asked.
 */
function ownerClause(ownerUserId: string | null, alias = ""): { sql: string; params: unknown[] } {
...
export class AnalyticsStore {
  ...
  async getSummary(filters: AnalyticsFilters): Promise<AnalyticsSummaryResponse> {
    ...
    const owner = ownerClause(filters.ownerUserId);
      ...
      .prepare(
        `SELECT
         ...
         WHERE created_at >= ? AND created_at < ?
           AND spawn_source IN (${placeholders})${owner.sql}`
      )
      .bind(filters.startAt, filters.endAt, ...sources, ...owner.params)
      .first<SummaryRow>();
  ...
  async getTimeseries(filters: AnalyticsFilters): Promise<AnalyticsTimeseriesResponse> {
    const sources = filters.spawnSources ?? HUMAN_SPAWN_SOURCES;
    const placeholders = sources.map(() => "?").join(", ");
    const owner = ownerClause(filters.ownerUserId, "s");
      ...
      .prepare(
        `SELECT
         ...
         WHERE s.created_at >= ? AND s.created_at < ?
           AND s.spawn_source IN (${placeholders})${owner.sql}

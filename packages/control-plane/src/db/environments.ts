...
export type EnvironmentScalarFields = Partial<
  Pick<EnvironmentRow, "name" | "description" | "channel_associations">
>;
...
export class EnvironmentStore {
    ...
    return this.db
      .prepare(
        `INSERT INTO environments
         (id, name, description, channel_associations, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
      .bind(
        row.id,
   ...
   * repository rows and secret rows. Sessions created from the environment
   * keep a benignly dangling environment_id (design §7.2).
   *

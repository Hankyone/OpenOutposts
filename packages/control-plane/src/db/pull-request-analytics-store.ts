...
export interface PullRequestAnalyticsFilters {
  ...
  now: number;
  /**
   * Restrict every metric to pull requests produced by sessions owned by this
   * user, or null for the deployment-wide view.
   *
   * Required, not optional: an omitted owner scope reads as "everyone", which
   * would expose other people's repository names, PR URLs and session cost.
   * See auth/analytics-scope.ts for who gets which value.
   */
  ownerUserId: string | null;
}

/**
 * Owner predicate for a query over `session_pull_requests`, resolved through
 * the producing session — the PR table carries no owner of its own. A null
 * scope contributes no predicate (deployment-wide).
 */
function prOwnerClause(ownerUserId: string | null, alias = ""): { sql: string; params: unknown[] } {
  if (ownerUserId === null) return { sql: "", params: [] };
  const prefix = alias ? `${alias}.` : "";
  return {
    sql: ` AND ${prefix}session_id IN (SELECT id FROM sessions WHERE user_id = ?)`,
    params: [ownerUserId],
  };
}

/** Owner predicate for a query whose FROM already reaches `sessions`. */
function sessionOwnerClause(
  ownerUserId: string | null,
  alias = ""
): { sql: string; params: unknown[] } {
  if (ownerUserId === null) return { sql: "", params: [] };
  const prefix = alias ? `${alias}.` : "";
  return { sql: ` AND ${prefix}user_id = ?`, params: [ownerUserId] };
}
...
export class PullRequestAnalyticsStore {
  ...
  async get(filters: PullRequestAnalyticsFilters): Promise<AnalyticsPullRequestsResponse> {
    ...
    // One owner scope, applied to every statement in the batch — a metric left
    // unscoped here would leak on its own, and its absence would be invisible
    // next to seven scoped siblings.
    const prOwner = prOwnerClause(filters.ownerUserId);
    const sessionOwner = sessionOwnerClause(filters.ownerUserId);
    const joinedSessionOwner = sessionOwnerClause(filters.ownerUserId, "s");
      ...
      this.db
        .prepare(
          `SELECT
             ...
             WHERE ${cohortWindow}${prOwner.sql}`
        )
        .bind(...cohortBinds, ...prOwner.params),
      this.db
        .prepare(
        ...
        .bind(...cohortBinds, ...sessionOwner.params),
      this.db
        .prepare(
          `SELECT
               COUNT(*) AS merged,
             ...
             FROM session_pull_requests
             WHERE lifecycle_state = 'merged' AND merged_at >= ? AND merged_at < ?${prOwner.sql}`
        )
        .bind(filters.startAt, filters.endAt, ...prOwner.params),
      this.db
        .prepare(
          `SELECT
               COUNT(*) AS total,
             ...
             FROM session_pull_requests
             WHERE lifecycle_state = 'open'${prOwner.sql}`
        )
        .bind(filters.now, ...prOwner.params),
      this.db
        .prepare(
      ...
      this.db
        .prepare(
          `SELECT
             ...
             WHERE ${prCreatedAtExpr("p")} >= ? AND ${prCreatedAtExpr("p")} < ?${joinedSessionOwner.sql}
             GROUP BY s.spawn_source
             ORDER BY created DESC, source ASC`
        )
        .bind(...cohortBinds, ...joinedSessionOwner.params),
    ]);

...
import { mcpServerEnvContext } from "../auth/encryption-contexts";
...
export class McpServerStore {
  ...
  private async encryptEnv(serverId: string, env: Record<string, string>): Promise<string> {
    const plain = JSON.stringify(env);
    if (!this.encryptionKey || Object.keys(env).length === 0) return plain;
    return encryptToken(plain, this.encryptionKey, mcpServerEnvContext(serverId));
  }

  private async decryptEnv(serverId: string, raw: string): Promise<Record<string, string>> {
    if (!this.encryptionKey) return safeJsonParseEnv(raw);
    try {
      const plain = await decryptToken(raw, this.encryptionKey, mcpServerEnvContext(serverId));
  ...
  private async decryptRow(row: McpServerRow): Promise<McpServerConfig> {
    const env = await this.decryptEnv(row.id, row.env);

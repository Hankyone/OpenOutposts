 ...
 *   { status: "stopped", createdAt: ..., hasActiveWebSocket: false },
 *   { cooldownMs: 30000, readyWaitMs: 60000, spawningTimeoutMs: 120000 },
 *   Date.now(),
 *   false
 * );
 * ```
 ...
 * - Short enough to avoid holding a machine for an abandoned session
...
export function evaluateInactivityTimeout(
  ...
  if (inactiveTime >= config.timeoutMs) {
    ...
    // No clients connected - time the session out
 ...
 * Boot sequence (lease, workspace preparation, harness start, bridge connect)
 * typically takes 30–90 seconds. Two minutes provides margin without leaving

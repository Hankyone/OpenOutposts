/**
 * SandboxLifecycleManager - orchestrates session execution lifecycle.
 *
 * This class coordinates spawn and timeout logic by:
 ...
 * 3. Delegating backend operations to the SandboxProvider abstraction
 *
 ...
 * runtime: an outpost is a machine the user owns and the product does not
 * manage its filesystem lifetime.
 */

import { extractProviderAndModel, type SandboxSettings } from "@open-inspect/shared";
...
export interface SandboxStorage {
  ...
  /** Update sandbox for spawn (status, both token hashes, sandbox ID, created_at) */
  ...
  /**
   * Clear the browser-access columns (code-server URL and password, tunnel
   * URLs, ttyd URL and token) left behind by pre-outpost sessions. Nothing
   * writes them any more — port exposure on a machine the user owns is its own
   * decision and is not carried forward from the sandbox model.
   */
  clearLegacySandboxAccessState(): void;
}
...
export class SandboxLifecycleManager implements SandboxLifecycle {
  ...
  /**
   * Hand the session to the execution backend.
   *
  ...
  private async doSpawn(): Promise<void> {
    ...
    let expectedSandboxId: string | null = null;
    ...
    try {
      ...
      // A second, narrower credential minted with the first and rotated with
      // it. The bridge token authorizes the session's whole callback surface;
      // this one reaches the model-credential broker and nothing else, which
      // is all the harness process needs to hold.
      const credentialFetchToken = this.idGenerator.generateId();
      expectedSandboxId = buildSandboxIdForSession(session, now);
      ...
      this.storage.updateSandboxForSpawn({
        ...
        credentialFetchTokenHash: await hashToken(credentialFetchToken),
      ...
      const createConfig: CreateSandboxConfig = {
        ...
        sandboxSettings,
        outpostId: session.outpost_id ?? null,
        ...multiRepoFields,
      };
      ...
      const spawnedGenerationStillOwnsState =
        currentSandbox?.modal_sandbox_id === expectedSandboxId &&
        currentSandbox.status === "spawning";
      if (!spawnedGenerationStillOwnsState) {
        // The provider may finish after a concurrent lifecycle decision.
        // Preserve that newer decision instead of reviving a stopped sandbox.
        this.log.info("Spawn completion preserved newer sandbox state", {
          expected_sandbox_id: expectedSandboxId,
          current_sandbox_id: currentSandbox?.modal_sandbox_id ?? null,
          current_status: currentSandbox?.status ?? null,
        });
        return;
      }
      ...
      const currentSandbox = this.storage.getSandbox();
      const failedGenerationStillOwnsState =
        expectedSandboxId !== null &&
        currentSandbox?.modal_sandbox_id === expectedSandboxId &&
        (currentSandbox.status === "spawning" || currentSandbox.status === "connecting");
      if (failedGenerationStillOwnsState) {
        ...
        // A concurrent lifecycle decision, especially an explicit cancel, is
        // authoritative. A late provider failure may be logged, but it must
        // not turn a stopped session back into an error.
        this.log.info("Spawn failure preserved newer sandbox state", {
          expected_sandbox_id: expectedSandboxId,
          current_sandbox_id: currentSandbox?.modal_sandbox_id ?? null,
          current_status: currentSandbox?.status ?? null,
        });
  ...
  async handleAlarm(): Promise<void> {
    ...
    if (connectingResult.isTimedOut) {
      ...
      this.storage.updateSandboxStatus("failed");
      this.storage.clearLegacySandboxAccessState();
    ...
    switch (inactivityDecision.action) {
      case "timeout":
        ...
        this.broadcaster.broadcast({
          ...
          message: "Session stopped due to inactivity",
        });
        return;
   ...
   *
   * The provider chosen here decides which of the user's API keys the sandbox
   * is handed. An id that names no provider used to be assumed to be
   * Anthropic's, so a session on an unrecognised model quietly asked for the
   * wrong key and failed later with no mention of the model. Now the spawn
   * fails here instead, and its message reaches the user: both callers are
   * inside the spawn/restore try blocks, which broadcast `sandbox_error`.
   */
  private resolveProviderAndModel(session: SessionRow): { provider: string; model: string } {
    const modelId = session.model || this.config.model;
    const resolved = extractProviderAndModel(modelId);
    if (!resolved) {
      throw new Error(
        `Cannot start this session: model "${modelId}" names no provider, so no ` +
          'credential can be resolved for it. Expected "provider/model-id".'
      );
    }
    return resolved;
  }

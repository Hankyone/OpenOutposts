/**
 * Direct REST client for the Daytona instance API.
 *
 * RETAINED FOR FLEET-MEMBER PROVISIONING. Daytona no longer hosts an agent:
 * the agent runs centrally and execution happens on a machine the user owns.
 * What survives here is the instance lifecycle — create, describe, start,
 * stop, recover — which the (not yet built) fleet-member creator will use with
 * the *user's own* Daytona credential to stand up a machine running the
 * outpost worker. Nothing in the control plane constructs this client today.
 *
 * Bearer token auth, per-operation timeouts, native fetch from Workers.
 */

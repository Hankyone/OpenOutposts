 ...
 *
 * RETAINED FOR FLEET-MEMBER PROVISIONING. Vercel no longer hosts an agent: the
 * agent runs centrally and execution happens on a machine the user owns. What
 * survives here is the instance lifecycle — create a session, run a command in
 * it, upload an archive, stop it — which the (not yet built) fleet-member
 * creator will use with the *user's own* Vercel credential to stand up a
 * machine running the outpost worker. Nothing in the control plane constructs
 * this client today.
 *
 * The snapshot calls went with the in-sandbox runtime: a snapshot exists only
 * because a session was pinned to a disposable filesystem.
 *
...
const USER_AGENT = "openoutposts/vercel-sandbox";

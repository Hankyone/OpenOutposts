/**
 * Type definitions for OpenOutposts Control Plane.
 */
...
export interface Env {
  ...
  OUTPOST: DurableObjectNamespace;
  HOMESTEAD?: DurableObjectNamespace; // Central homestead registry (outpost execution path)
  ...
  // Worker release distribution. Optional because a deployment that has not
  // yet applied the Terraform for it still serves everything else; the
  // /releases route answers 503 rather than the Worker failing to boot.
  ...
  INTERNAL_CALLBACK_SECRET?: string; // For signing callbacks to slack-bot and the outpost/homestead handshakes
  OUTPOST_ENROLLMENT_TOKEN?: string; // Legacy shared token; ignored unless compatibility is enabled
  OUTPOST_ALLOW_LEGACY_SHARED_TOKEN?: string; // Explicit "true" enables legacy claiming/connects
  OUTPOST_TARGET_ID?: string; // Outpost hosting session workspaces when SANDBOX_PROVIDER=outpost
  OUTPOST_TARGET_WORKSPACE_ROOT?: string; // Directory on that outpost for per-session workspaces
  OUTPOST_CLONE_URL_TEMPLATE?: string; // Clone URL template with {owner}/{name} (default GitHub https)
  ...
  SERVICE_AUTH_SECRET_HOMESTEAD?: string;
  ...
  // Session transcript retention, read by SessionDO. Each falls back to its
  // default when absent or not a positive whole number.
  SESSION_EVENT_PAYLOAD_MAX_BYTES?: string; // Per-event stored payload cap (default: 65536)
  SESSION_EVENTS_MAX_COUNT?: string; // Events kept before oldest-first pruning (default: 20000)
  SESSION_EVENTS_MAX_BYTES?: string; // Stored payload bytes kept (default: 52428800 = 50 MiB)

...
export function deriveIdentity(principal: Principal | undefined): DerivedIdentity | null {
  ...
  switch (principal.kind) {
    ...
    case "service":
      ...
      // Neither of these acts for a person. The homestead is the execution
      // side: it is handed a session that already has an owner and never
      // originates one, so it asserts no actor and is not a spawn source.
      if (principal.service === "modal" || principal.service === "homestead") {

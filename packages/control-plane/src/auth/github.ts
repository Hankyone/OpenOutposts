...
import { scmOAuthContext } from "./encryption-contexts";
...
/**
 * Store encrypted GitHub tokens, sealed to the GitHub user they belong to —
 * the same binding `UserScmTokenStore` and the session participant rows use,
 * so a value produced here is readable wherever that identity's tokens are
...
export async function encryptGitHubTokens(
  ...
  const accessTokenEncrypted = await encryptToken(
    tokens.access_token,
    encryptionKey,
    scmOAuthContext(providerUserId, "access_token")
  );
  ...
  const refreshTokenEncrypted = tokens.refresh_token
    ? await encryptToken(
        ...
        encryptionKey,
        scmOAuthContext(providerUserId, "refresh_token")
      )
    : null;
...
export async function getValidAccessToken(
  ...
  if (stored.expiresAt && stored.expiresAt - now < bufferMs) {
    ...
    const newStored = await encryptGitHubTokens(newTokens, config.encryptionKey, providerUserId);

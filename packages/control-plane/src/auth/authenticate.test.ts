...
const SECRETS = {
  ...
  SERVICE_AUTH_SECRET_HOMESTEAD: "homestead-secret",
...
const SERVICE_SECRET: Record<ServiceName, string> = {
  ...
  homestead: SECRETS.SERVICE_AUTH_SECRET_HOMESTEAD,
...
function createCtx(identityRow: Record<string, unknown> | null = null): RequestContext {
  ...
  return {
    ...
    request_id: "req-test",
    metrics: { summarize: () => ({}) },
    db: { prepare: vi.fn(() => statement), batch: vi.fn(async () => []), exec: vi.fn() },
...
describe("authenticate — nonce replay", () => {
  /**
   * A context whose nonce store enforces the primary key the migration
   * declares. A stub that accepts every insert cannot tell a first delivery
   * from a replay, so it cannot test one.
   */
  function createCtxWithNonceStore(): { ctx: RequestContext; spent: Set<string> } {
    ...
    const statement = (query: string) => {
      ...
      Object.defineProperty(stmt, "__apply", {
        value: () => {
          if (!query.startsWith("INSERT INTO service_auth_nonces")) return;
          const key = String(bound[0]);
          if (spent.has(key)) throw new Error("UNIQUE constraint failed");
          spent.add(key);
        },
      });
      return stmt;
    ...
    const ctx = {
      ...
      db: {
        ...
        batch: vi.fn(async (statements: Array<Record<string, unknown>>) => {
          for (const s of statements) (s.__apply as () => void)();
          return statements.map(() => ({ results: [], meta: {} }));
        }),
        exec: vi.fn(),
      },
    } as unknown as RequestContext;
  ...
  it("rejects a replayed signature inside the validity window", async () => {
    const { ctx, spent } = createCtxWithNonceStore();
    const url = "https://cp.test.local/sessions";
    const body = "{}";
    const headers = await buildServiceAuthHeaders({
      service: "modal",
    ...
    // The signature is still inside its validity window and still verifies.
    // Only the spent nonce stands between it and a second acceptance.
  ...
  it("refuses rather than admits when the nonce store is unavailable", async () => {
    const ctx = {
      trace_id: "trace-test",
      request_id: "req-test",
      metrics: { summarize: () => ({}) },
    ...
    const result = await authenticate(await signedRequest({ service: "modal" }), createEnv(), ctx);

    // A nonce we could not record is a nonce we cannot promise is unspent.
    expect(result).toEqual({
      reason: "Unauthorized",
      status: 401,
      failedScheme: "per-service",

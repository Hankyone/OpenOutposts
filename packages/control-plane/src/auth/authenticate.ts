...
import { consumeServiceNonce } from "./service-nonce";
...
export function serviceAuthSecret(env: ServiceKeyEnv, service: ServiceName): string | undefined {
  switch (service) {
    ...
    case "homestead":
      return env.SERVICE_AUTH_SECRET_HOMESTEAD;
    case "modal":
...
async function authenticateServiceCredential(
  ...
  if (
    !(await consumeServiceNonce(ctx.db, service, verification.nonce, logger, {

...
A stateless Cloudflare Worker that translates GitHub webhook events into OpenOutposts coding agent
...
reaction, creates a session via the control plane, and sends a prompt. The central agent handles
GitHub interaction (posting reviews, comments, and pushing code) through the leased outpost.
...
### Outpost Prerequisites

The agent no longer runs inside a provisioned sandbox: it runs centrally and every file and shell
operation executes on an outpost the user owns. For the agent to interact with GitHub from there,
the machine needs a `gh` CLI and git credentials.

Re-homing the git credential helper and the `gh` wrapper — which lived in the in-sandbox runtime and
brokered short-lived SCM credentials from the control plane — onto the outpost is outstanding work.
Until it lands, git and `gh` on an outpost use whatever credentials exist on that machine.

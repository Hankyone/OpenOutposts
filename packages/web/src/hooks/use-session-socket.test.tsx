...
describe("useSessionSocket", () => {
  ...
  it("routes a repo-scoped session_branch to the matching member, mirroring the scalar only for the primary", async () => {
    ...
    expect(result.current.sessionState?.repositories?.[0].branchName).toBe(
      "openoutposts/session-1"
    );
    expect(result.current.sessionState?.branchName).toBe("openoutposts/session-1");
    ...
    await waitFor(() => {
      expect(result.current.sessionState?.branchName).toBe("openoutposts/session-1-web");
    });
    expect(result.current.sessionState?.repositories?.[0].branchName).toBe(
      "openoutposts/session-1-web"
    );
  });
  ...
  it("ignores an unscoped session_branch for a multi-repo session", async () => {
    ...
    act(() => {
      socket.receive({ type: "session_branch", branchName: "openoutposts/session-1-orphan" });
    });

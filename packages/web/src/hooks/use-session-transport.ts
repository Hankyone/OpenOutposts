...
function closeDirective(
  ...
  // A clean close is still a lost session view, and nothing reconnects on its
  // own. Sockets discarded by reconnect() and unmount never reach here — the
  // identity guard drops them — so this is always a close the user must be
  // told about and offered a way back from.
  if (event.wasClean) {
...
export function useSessionTransport(
  ...
  const handleSocketClose = useCallback((ws: WebSocket, event: CloseEvent, retry: () => void) => {
    ...
    switch (directive.action) {
      ...
      case "closed":
        setConnectionError("Connection closed. Reconnect to continue.");
        return;
    }
  }, []);

...
describe("useSessionTransport", () => {
  ...
  it("surfaces a clean close as a recoverable error instead of going quiet", async () => {
    const { result, socket } = await openSocket();
    ...
    await waitFor(() => {
      expect(result.current.connectionError).toBe("Connection closed. Reconnect to continue.");
    });
    // Nothing reconnects on its own after a clean close.
    expect(FakeWebSocket.instances).toHaveLength(1);
  ...
  it("stays quiet when unmounting closes the socket", async () => {
    const { result, unmount } = await openSocket();

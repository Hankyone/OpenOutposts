...
describe("sessionSocketReducer", () => {
  ...
  describe("session_branch", () => {
    ...
    it("routes a repo-scoped update to the matching member, mirroring the scalar only for the primary", () => {
      ...
      const base = subscribedState({
        state: createSessionState({ branchName: "openoutposts/session-1", repositories }),
      });
      ...
      const secondary = reduce(
        ...
        serverMessage({
          type: "session_branch",
          branchName: "openoutposts/session-1-api",
          repoOwner: "acme",
          repoName: "api",
        })
      );
      expect(secondary.sessionState?.repositories?.[1].branchName).toBe(
        "openoutposts/session-1-api"
      );
      expect(secondary.sessionState?.branchName).toBe("openoutposts/session-1");

      const primary = reduce(
        secondary,
        serverMessage({
          type: "session_branch",
          branchName: "openoutposts/session-1-web",
          repoOwner: "acme",
          repoName: "web",
        })
      );
      expect(primary.sessionState?.repositories?.[0].branchName).toBe("openoutposts/session-1-web");
      expect(primary.sessionState?.branchName).toBe("openoutposts/session-1-web");
  ...
  describe("local actions", () => {
    ...
    it("stops reporting a turn as processing once the socket is gone", () => {
      // The turn's end only ever arrives over this socket, so a mid-turn drop
      // would otherwise spin the thinking indicator forever.
      const processing = reduce(subscribedState(), { type: "prompt_sent" });
      expect(processing.sessionState?.isProcessing).toBe(true);

      const closed = reduce(processing, { type: "socket_closed" });
      expect(closed.sessionState?.isProcessing).toBe(false);
      expect(closed.replaying).toBe(false);
    });

    it("restores the server's own processing verdict on the next subscribe", () => {
      const closed = reduce(subscribedState(), { type: "prompt_sent" }, { type: "socket_closed" });

      const resubscribed = reduce(
        closed,
        serverMessage(
          createSubscribedMessage({ state: createSessionState({ isProcessing: true }) })
        )
      );
      expect(resubscribed.sessionState?.isProcessing).toBe(true);
    });

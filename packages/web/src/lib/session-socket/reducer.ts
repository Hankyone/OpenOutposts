...
export function sessionSocketReducer(
  ...
  switch (action.type) {
    ...
    case "socket_closed":
      // A turn only ends on the wire: `processing_status` and
      // `execution_complete` both arrive over this socket. Once it is gone the
      // client can never learn that the turn finished, so leaving the flag set
      // spins the thinking indicator forever. The next `subscribed` replaces
      // the whole projection with the server's own view.
      return updateSessionState({ ...state, replaying: false }, (prev) => ({
        ...prev,
        isProcessing: false,
      }));
  }

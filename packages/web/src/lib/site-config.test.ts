...
describe("site-config", () => {
  ...
  it("falls back to OpenOutposts when NEXT_PUBLIC_APP_NAME is unset", async () => {
    delete process.env.NEXT_PUBLIC_APP_NAME;
    const { APP_NAME } = await import("./site-config");
    expect(APP_NAME).toBe("OpenOutposts");
  });

  it("falls back to OpenOutposts when NEXT_PUBLIC_APP_NAME is empty", async () => {

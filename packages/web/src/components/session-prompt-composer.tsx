...
type SessionPromptComposerProps = {
  ...
  prompt: {
    ...
    draftLocked: boolean;
    /**
     * Why a send cannot go through right now, or null when it can. The send
     * control is disabled exactly when this is set and repeats it on hover, so
     * what the button looks like and what it will do cannot drift apart.
     */
    sendBlockedReason: string | null;
    /** A send that was attempted and did not go through. */
  ...
  model: {
    ...
    items: ComboboxGroup[];
    /** Nothing the harness catalog offers is reachable for this user. */
    needsProviderConnection: boolean;
    /**
     * The selected model's reasoning support as the harness reports it, or
     * undefined when the catalog does not name the model and the bundled table
...
export function SessionPromptComposer({
  ...
  const sendDisabled =
    !hasContent || prompt.isProcessing || prompt.draftLocked || prompt.sendBlockedReason !== null;
  const sendLabel =
    prompt.isProcessing && hasContent
      ? "Wait for execution to complete"
      : (prompt.sendBlockedReason ?? `Send (${SHORTCUT_LABELS.SEND_PROMPT})`);
  ...
  return (
    <footer className="min-w-0 border-t border-border-muted flex-shrink-0">
      <form onSubmit={prompt.onSubmit} className="w-full min-w-0 max-w-4xl mx-auto p-4 pb-6">
        ...
        {/* Why a typed draft cannot be sent, said before the user tries */}
        {(prompt.sendError || (prompt.sendBlockedReason && hasContent)) && (
          <p
            role={prompt.sendError ? "alert" : "status"}
            className={`mb-2 text-xs ${prompt.sendError ? "text-destructive" : "text-muted-foreground"}`}
          >
            {prompt.sendError ?? prompt.sendBlockedReason}
          </p>
        )}
          ...
          <div className="flex flex-col gap-2 px-4 py-2 border-t border-border-muted sm:flex-row sm:items-center sm:justify-between sm:gap-0">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 min-w-0">
              <ModelPicker
                ...
                onSelect={model.onModelChange}
                items={model.items}
                disabled={prompt.isProcessing || prompt.draftLocked}
                needsProviderConnection={model.needsProviderConnection}
              />
              ...
              <ReasoningEffortPills
                ...
                disabled={prompt.isProcessing || prompt.draftLocked}
                catalogReasoning={model.catalogReasoning}
              />
            </div>

...
interface MetadataSectionProps {
  ...
  /** Machine the session executes on; omitted by callers that don't know. */
...
const OUTPOST_REACHABILITY_TEXT: Record<string, { text: string; className: string }> = {
  connected: { text: "connected", className: "text-success" },
  stale: { text: "no heartbeat", className: "text-warning" },
  offline: { text: "offline", className: "text-destructive" },
  unknown: { text: "connection unknown", className: "text-muted-foreground" },
};

/**
 * The machine this session's file and shell work runs on, with the heartbeat
 * the control plane last saw from it. A machine the fleet listing does not
 * describe is still named — only its connection state is withheld.
 */
function OutpostRow({ target }: { target: Extract<SessionExecutionTarget, { kind: "outpost" }> }) {
  const reachability = OUTPOST_REACHABILITY_TEXT[target.reachability];
  const lastSeen = target.lastSeenAt === null ? null : formatRelativeTime(target.lastSeenAt);

  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2 text-sm">
        <MonitorIcon className="w-4 h-4 text-muted-foreground" />
        <span className="text-foreground truncate max-w-[180px]" title={target.outpostId}>
          {target.name}
        </span>
      </div>
      <div className="ml-6 flex items-center gap-2 text-xs">
        <span className={reachability.className}>{reachability.text}</span>
        {target.reachability !== "connected" && lastSeen && (
          <span className="text-muted-foreground">
            {lastSeen === "just now" ? "seen just now" : `seen ${lastSeen} ago`}
          </span>
        )}
        {target.workerVersion && (
          <span className="text-muted-foreground">worker {target.workerVersion}</span>
        )}
      </div>
    </div>
  );
...
export function MetadataSection({
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Machine the session executes on */}
      {executionTarget?.kind === "outpost" && <OutpostRow target={executionTarget} />}

...
describe("MetadataSection", () => {
  ...
  it("names the machine the session runs on with its heartbeat and worker version", () => {
    render(
      <MetadataSection
        createdAt={Date.now()}
        baseBranch="main"
    ...
    expect(screen.getByText("Workshop Mac")).toBeInTheDocument();
    expect(screen.getByText("connected")).toBeInTheDocument();
    expect(screen.getByText("worker 0.4.1")).toBeInTheDocument();
  });

  it("shows when the machine was last seen once it stops answering", () => {
    render(
      <MetadataSection
        createdAt={Date.now()}
        baseBranch="main"
        executionTarget={{
          ...
          lastSeenAt: Date.now() - 20 * 60 * 1000,
          platform: "darwin",
          workerVersion: null,
        }}
      />
    ...
    expect(screen.getByText("seen 20m ago")).toBeInTheDocument();
  });

  it("renders no machine row while the session names no machine", () => {
    render(
      <MetadataSection
        createdAt={Date.now()}
        baseBranch="main"

...
describe("WebSessionGate", () => {
  ...
  it("retries a retryable 401 instead of destroying a live session", async () => {
    // A cookie read landing mid rotation-write reports no session for one
    // request. Signing out there is unrecoverable, so the blip must be retried.
    mocks.status = "authenticated";
    fetchSpy
      .mockResolvedValueOnce(
        Response.json({ error: "Unauthorized", reason: "no_session" }, { status: 401 })
      )
    ...
    expect(
      await screen.findByText("Protected application", undefined, { timeout: 3000 })
    ).toBeTruthy();
    expect(mocks.signOut).not.toHaveBeenCalled();
    expect(fetchSpy).toHaveBeenCalledTimes(2);
  });

  it("signs out once the retries are exhausted", async () => {
    mocks.status = "authenticated";
    ...
    // Initial attempt plus both backoff retries.
    expect(fetchSpy).toHaveBeenCalledTimes(3);
  });

  it("signs out immediately when the grant is terminally invalid", async () => {
    // Rotation reuse is the token-theft signal — retrying it would be wrong.
    mocks.status = "authenticated";
    fetchSpy.mockResolvedValue(
      Response.json({ error: "Unauthorized", reason: "grant_invalid" }, { status: 401 })
    );

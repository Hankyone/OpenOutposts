  ...
  return (
    <div className="py-0.5">
      <button
        type="button"
        onClick={onToggle}
        className={`w-full flex items-center gap-1.5 text-sm text-left transition-colors ${
          formatted.failed
      ...
      >
        ...
        />
        {formatted.failed ? (
          <ErrorIcon className="w-3.5 h-3.5 flex-shrink-0 text-destructive" />
        ) : (
        ...
        {formatted.failed && <span className="flex-shrink-0 text-xs">failed</span>}
        {showTime && (
      ...
      {isExpanded && (
        <div className="mt-2 ml-5 p-3 bg-card border border-border-muted text-xs overflow-hidden">
          ...
          {/* Replayed from a stored copy the control plane had to shorten.
              Without saying so, the fragment reads as the whole output. */}
          {event.truncated && (
            <div className="mt-2 text-secondary-foreground">Output truncated for storage</div>
          )}
        </div>
      )}
    </div>

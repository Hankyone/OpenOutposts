...
/**
 * Identity for one event, built only from what the event carries. A running
 * index would do, but `history_page` prepends older events and shifts every
 * later index — remounting every group below it, which collapses expanded tool
 * groups, resets copy state, and makes the scroll anchor fight a full relayout.
 */
function eventIdentity(event: SandboxEvent): string {
  const ackId = "ackId" in event ? event.ackId : undefined;
  if (ackId) return `ack:${ackId}`;
  const callId = "callId" in event ? event.callId : undefined;
  if (callId) return `call:${callId}`;
  const messageId = "messageId" in event ? event.messageId : undefined;
  return `${event.type}:${messageId ?? ""}:${event.timestamp}`;
}

function groupEvents(events: SandboxEvent[]): EventGroup[] {
  const groups: EventGroup[] = [];
  let currentToolGroup: ToolCallEvent[] = [];
  // Two events can share an identity (same type, message and second). The
  // suffix disambiguates them and stays put under prepending, because it only
  // counts prior events carrying that same identity.
  const seenIdentities = new Map<string, number>();

  const uniqueKey = (prefix: string, event: SandboxEvent) => {
    const identity = eventIdentity(event);
    const seen = seenIdentities.get(identity) ?? 0;
    seenIdentities.set(identity, seen + 1);
    return `${prefix}-${identity}${seen > 0 ? `#${seen}` : ""}`;
  };
  ...
  const flushToolGroup = () => {
    if (currentToolGroup.length > 0) {
      groups.push({
        ...
        id: uniqueKey("tool-group", currentToolGroup[0]),
      });
      currentToolGroup = [];
    }
  };
  ...
  for (const event of events) {
    if (event.type === "tool_call") {
      ...
      flushToolGroup();
      groups.push({ type: "single", event, id: uniqueKey("single", event) });
    }
  }
...
/** "owner/name branch", dropping whichever part the event did not carry. */
function pushScope(event: {
  repoOwner?: string;
  repoName?: string;
  branchName?: string;
}): string | null {
  const repo = event.repoOwner && event.repoName ? `${event.repoOwner}/${event.repoName}` : null;
  return [repo, event.branchName].filter(Boolean).join(" ") || null;
}

function PushCompleteEvent({ event }: EventRendererProps) {
  if (event.type !== "push_complete") return null;

  const scope = pushScope(event);
  return (
...
function PushErrorEvent({ event }: EventRendererProps) {
  if (event.type !== "push_error") return null;

  const scope = pushScope(event);
  return (
    <StatusRow tone="destructive" time={formatEventTime(event)}>
      {scope ? `Push failed for ${scope}: ${event.error}` : `Push failed: ${event.error}`}
    </StatusRow>
  );
}
...
> = {
  ...
  push_complete: PushCompleteEvent,
  push_error: PushErrorEvent,
};

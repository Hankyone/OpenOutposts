# OpenOutposts Linear Agent

Cloudflare Worker that integrates [Linear](https://linear.app) with OpenOutposts as a first-class
...
```
@OpenOutposts on issue → Linear sends AgentSessionEvent webhook →
  Agent emits "Thinking..." → Resolves repo → Creates session →
  Prompt reaches the homestead → Eligible issue may move to team's lowest-position started state →
  Agent emits "Working on owner/repo..." → Agent codes through the leased outpost →
...
5. Creates an OpenOutposts coding session and sends the issue as a prompt
6. After a human-initiated prompt reaches a live homestead session, moves an eligible issue to the
   team's lowest-position `started` workflow state when the signed callback is delivered and
   accepted
...
session rather than creating a new one. Stopping or cancelling the agent in Linear stops the
session's active outpost work.
...
- **Application name:** `OpenOutposts` (this is how the bot appears in @mentions)
...
After installation, `@OpenOutposts` will appear in the mention and assignee menus.
...
In the OpenOutposts web UI, go to **Settings → Integrations → Linear** to configure:
...
- Type `@OpenOutposts` in a comment → agent picks up the issue
- Assign the issue to `OpenOutposts` → agent picks it up

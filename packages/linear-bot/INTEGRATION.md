...
3. Agent creates an OpenOutposts session and sends the prompt
4. When a human-initiated prompt is dispatched to a live homestead session, the control plane sends
   a signed start callback and the agent moves an eligible issue to the team's lowest-position
